...
public class TwitterFormatter implements PlatformFormatter {
    ...
    private List<String> extractParagraphs(Node node) {
        List<String> paragraphs = new ArrayList<>();
        node.accept(new NodeVisitor() {
            @Override
            public void visit(Node n) {
                if (n instanceof Paragraph || n instanceof Heading) {
        ...
        for (String paragraph : paragraphs) {
            ...
            for (String sentence : sentences) {
                ...
                if (currentChunk.length() + sentence.length() + 1 <= MAX_TWEET_LENGTH - reservedSuffixLength) {
                    ...
                    // If single sentence exceeds limit, we'd need AI condenser (placeholder)
